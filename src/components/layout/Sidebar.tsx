...
export function Sidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <aside className={cn(
      ...
      {/* Quick Actions */}
      <div className="flex items-center gap-2 p-3">
        <button className="flex-1 flex items-center gap-2 px-3 py-2 text-sm text-text-secondary bg-background-tertiary rounded-lg hover:bg-hover transition-colors">
          <Search className="w-4 h-4" />
          {!collapsed && "Search..."}
          {!collapsed && <kbd className="ml-auto text-xs text-text-tertiary">⌘K</kbd>}
        </button>
      ...
      {!collapsed && (
        <div className="border-t border-border p-3">
          <p className="text-xs font-medium text-text-tertiary uppercase tracking-wider mb-2">Favorites</p>
          <div className="space-y-1">
            {favorites.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="flex items-center gap-3 px-3 py-2 text-sm text-text-secondary rounded-lg hover:text-white hover:bg-hover transition-colors"
              >
                <item.icon className="w-4 h-4" />
                <span>{item.name}</span>
              </Link>
      ...
      <button
        onClick={onToggle}
        className="absolute -right-3 top-1/2 -translate-y-1/2 p-1 bg-background-tertiary border border-border rounded-full hover:bg-hover transition-colors"
      >
        {collapsed ? <ChevronRight className="w-4 h-4" /> : <ChevronLeft className="w-4 h-4" />}
      </button>
    </aside>
